...
public class BolsaTrabajo implements Serializable {
	...
	/*
	 * Se encarga de verificar que existe un usuario que concuerde con los datos
	 * recibidos.
	 * 
	...
	/*
	 * Se encarga de listar las solicitudes segun si referencian a la persona
	 * recibida.
	 * 
	...
	/*
	 * !Buscamos la Empresa que ha iniciado la seccion, asi optenemos la empresa que
	 * esta loggeada.
	 */
	private Institucion obtenerEmpresaSeccion() {
		...
		for (Usuario users : usuarios) {
			...
			if (users.getMyInstitucion() != null
					&& users.getMyInstitucion().getNombre().equalsIgnoreCase(usuarioActual.getMyInstitucion().getNombre()))
	...
	/*
	 * !Calculamos la coincidencia con de las solicitudes enviadas a la oferta dada
	 * devolviendo
	...
	/*
	 * !Sumamos todas laas coincidencia que se a encontrado en ese solicitante y la
	 * oferta realizada
	 */
	...
	/*
	 * !Verificamos que clase es la persona que esta solicitando y devolvemos y
	 * String para comparar con la oferta
	 */
	...
	/*
	 * !Busca la imagen en el buscardor del sistema operativo, la imagen
	 * seleccionada, se guarda en una
	...
	public ArrayList<Oferta> getOfertasRecomendadasByPersona(Persona persona) {
		...
		for (Oferta actual : this.ofertas) {
			if (actual.isEstado() && actual.getPais().equalsIgnoreCase(persona.getPais())
					&& actual.getAniosExperiencia() == persona.calcularAniosExperiencia())
				result.add(actual);
		}
		return result;
	}
